...
export default function ButtonOpacity({children,onPress,variant,type,outline,title,styleProps}){
    ...
    let backgroundColor = (variant==='primary' ? Colors.primary400 : (variant==='info' ? Colors.info500 : (variant==='success' ? Colors.success500 : (variant==='warning' ? Colors.warning500 : (variant==='danger' ? Colors.danger500 : (variant==='default' ? Colors.gray : ''))))))
